...
func testTwoOperandOp(t *testing.T, tests []TwoOperandTestcase, opFn executionFunc, name string) {
	var (
		env            = NewEVM(BlockContext{}, TxContext{}, nil, params.TestChainConfig)
		stack          = newstack()
		pc             = uint64(0)
...
func TestOpTstore(t *testing.T) {
	var (
		...
		env            = NewEVM(BlockContext{}, TxContext{}, statedb, params.TestChainConfig)
		stack          = newstack()
		mem            = NewMemory()
		evmInterpreter = NewEVMInterpreter(env)
...
func TestRandom(t *testing.T) {
	...
	for _, tt := range []testcase{
		...
		var (
			env            = NewEVM(BlockContext{Random: &tt.random}, TxContext{}, nil, params.TestChainConfig)
			stack          = newstack()
			pc             = uint64(0)
			evmInterpreter = env.interpreter
		)
...
func TestBlobHash(t *testing.T) {
	...
	for _, tt := range []testcase{
		...
		var (
			env            = NewEVM(BlockContext{}, TxContext{BlobHashes: tt.hashes}, nil, params.TestChainConfig)
			stack          = newstack()
			pc             = uint64(0)
			evmInterpreter = env.interpreter
		)
